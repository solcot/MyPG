...
def get_postgres_data():
    ...
    stdin, stdout, stderr = client.exec_command("psql -P pager=off -f value_engine.sql_see_S")
...
6. 10년 역사적 통계 및 연도별 피벗 데이터 (Historical Trend)
    ...
    * 선정된 종목의 아래 지표 표시
    - market_cap_백억 : trade_value_천만 : 영업이익률_pct : 시총대비_매출액_pct : 시총대비_영업이익_pct : 순부채_억 : change_rate : curr_price
       ▶ 지표값 ...
    - dividend_yield : pbr : per : roe : forward_per : forward_roe
       ▶ 지표값 ...
    - psr : peg : min_roe_ever : avg_roe_ever : max_roe_ever : hist_avg_pbr : hist_avg_dividend   
       ▶ 지표값 ...
...
def send_email(content):
    ...
    msg['Subject'] = "📈 [S]오늘의 Top 5 추천 종목 (AI 분석)"
