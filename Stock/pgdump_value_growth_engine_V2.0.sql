...
- 조건: 내 평단가 대비 -15% / -30%(25.33%) 하락 시
- 사용 쿼리: value_mool_monitor.sql (재조회)
- 액션: 쿼리에 '여전히 존재하면' 추가 매수. 만약 사라졌다면 물타기 즉시 중단.
  > 1차 물타기: 700 만 원 (자금이 충분할경우: 750만 원 or 1,000만 원)
  > 2차 물타기: 800 만 원 (자금이 충분할경우: 1,000만 원 or 1,500만 원)
...
성장기업 적정주가 = eps * (8.5 + 2*기대성장률)
정체기업 적정주기 = eps * (8.5 + 2*0)
  
*** 제 추천 ***

현재 시스템을 계속 발전시킨다면 저는 아래처럼 할 것 같습니다.

- 모델	추천도
 BPS + ROE + PBR	        ⭐⭐⭐⭐⭐                   : 현재 BPS -> 10년 성장(ROE) -> 10년 후 BPS -> 예상 PBR -> 10년 후 예상주가
 EPS + EPS 성장률 + PER	    ⭐⭐⭐ (보조지표)
 DCF	                    ⭐⭐⭐⭐ (데이터 확보가 어려움)

즉, 현재의 BPS 기반 모델을 메인으로 유지하는 것을 추천합니다.
즉, 안전마진 10%는 "관심 종목" 정도, 20% 이상부터 실제 매수를 진지하게 검토, 30% 이상이면 매우 매력적인 가격으로 판단하는 것이 일반적인 가치투자 기준에 더 가깝습니다.

--------> 안전마진은 가격의 상태를 알려주고, 예상 CAGR은 앞으로의 투자 매력을 알려주는 역할
매수
안전마진 20% 이상
예상 CAGR 15% 이상

보유
안전마진이 **0%~-20%**까지 내려가더라도,
ROE가 유지되고,
적정가가 계속 상승하며,
예상 CAGR이 여전히 목표(예: 12~15%) 이상이면 계속 보유

매도
안전마진이 -20~-30% 이하가 되었고,
동시에 예상 CAGR도 목표 수익률 아래(예: 10% 이하)로 떨어졌을 때
*******************************************************************************
...
last_data AS (
    SELECT 
        ROUND(((a.eps - p.past_eps) / NULLIF(ABS(p.past_eps), 0)) * 100, 2) AS today_eps_ratio,
...
cat > value_mool_monitor.sql <<'EEOFF'
WITH max_date_cte AS (
    SELECT MAX(trade_date) AS max_date FROM stockfdt_pbr_v
),
past_eps_cte AS (
...
Phase 2. 물타기 방어선 (Scale-In)
- 조건: 내 평단가 대비 -15% / -30%(25.33%) 하락 시
- 사용 쿼리: growth_mool_monitor.sql (재조회)
...
cat > growth_mool_monitor.sql <<'EEOFF'
WITH max_date_cte AS (
    SELECT MAX(trade_date) AS max_date FROM stockfdt_pbr_v
),
past_eps_cte AS (
